...
getLearningRate <- function(xn, xn1, devFxn, devFxn1, lr = 0.001, 
                            maxIter = 100 ,s = 1, beta = 0.5, sigma = 0.001, 
                            type = c("Specified", "B-BMethod", "Armijo"))
{
  if(type == "Specified")
  {
    lr <- as.numeric(lr);
  ...
  if(type == "Armijo")
  {
    alpha = s;
    iterNum = 1;
    while(iterNum < maxIter && 0 < -sigma * alpha * crossprod(devFxn))
    {
      alpha <- alpha * beta;
      iterNum <- iterNum + 1;
    }
