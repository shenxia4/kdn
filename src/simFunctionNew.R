                    ...
                    nSamp = 1e3, niter = 100, tol = 1e-5)   # these variables are used to assign initial values to the parameters
{
  ...
  for(i in 1:nS)
  {
    ...
    # Generate trait list
    traitList <- traitSim(mu = traitMu, Sigma = traitSigma, sigmaR = sigmaR, phi = phi, order = order, fun = traitFun);
  ...
  result <- matrix(c(mean(predErr), sd(predErr), mean(predErr.BLUPK), sd(predErr.BLUPK), mean(predErr.BLUPI), sd(predErr.BLUPI)), nrow = 1);
  colnames(result) <- c("Pred.Err.Mean", "Pred.Err.SD", "Pred.Err.Ker.Mean", "Pred.Err.Ker.SD", "Pred.Err.Id.Mean", "Pred.Err.Id.SD");
  return(result)
